...
public class InternalToRestExceptionHandler {
  ...
  ResponseEntity<RestException> handleMethodArgumentNotValid(MethodArgumentNotValidException e) {
    log.info("Invalid argument", e);
    return handleBadRequest(new BadRequestException(e.getMessage()));
  }

  @ExceptionHandler(value = {MissingServletRequestParameterException.class})
  ResponseEntity<RestException> handleMissingParameter(MissingServletRequestParameterException e) {
  ...
  ResponseEntity<RestException> handleUnauthorized(UnauthorizedException e) {
  ...
  ResponseEntity<RestException> handleConflict(ConflictException e) {
    log.info("Conflict", e);
  ...
  ResponseEntity<RestException> handleLockAcquisitionException(Exception e) {
  ...
  ResponseEntity<RestException> handleDefault(Exception e) {
    log.error("Internal error", e);
    return new ResponseEntity<>(
        toRest(e, HttpStatus.INTERNAL_SERVER_ERROR), HttpStatus.INTERNAL_SERVER_ERROR);
  }

  private RestException toRest(Exception e, HttpStatus status) {
    return RestException.builder().type(status.toString()).message(e.getMessage()).build();
  }
}
